...
impl Engine {
    ...
    fn place_in_display(&mut self, id: WindowId, action: Action, snapshot: &Snapshot) -> Option<Rect> {
    ...
    fn move_display(&mut self, id: WindowId, action: Action, snapshot: &Snapshot) -> Option<Rect> {
        ...
        let target =
            layout::remap_between(&snapshot.window, &snapshot.visible_frames[current], &snapshot.visible_frames[dest]);
        self.windows.entry(id).or_insert(WindowState {
            original: snapshot.window,
        });
        Some(target)
    ...
    fn target_rect(&self, action: Action, snapshot: &Snapshot) -> Option<Rect> {
        ...
        let target = layout::place(action, 0.5, &snapshot.window, &visible_frame, self.config.gaps)?;
        Some(target)
    }
}
